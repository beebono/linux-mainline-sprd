...
static int audio_sipc_probe(struct platform_device *pdev)
{
	...
	ret = aud_ipc_init(target_id);
	if (ret) {
		/*
		 * The IRAM/DDR pools come from audio-mem-mgr; if it has not
		 * probed yet, retry later instead of silently coming up
		 * with no shared-memory rings.
		 */
		dev_warn(&pdev->dev, "audio shmem not ready, deferring\n");
		return -EPROBE_DEFER;
	}

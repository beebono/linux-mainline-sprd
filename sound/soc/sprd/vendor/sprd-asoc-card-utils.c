			    ...
			    int *args_count)
{
	...
	/* Get dai->name */
	ret = snd_soc_of_get_dai_name(np, &component->dai_name, 0);
	if (ret < 0) {
				       ...
				       char *prefix, int idx)
{
	...
	if (strlen(prefix) && !bitclkmaster && !framemaster) {
		...
		daifmt = snd_soc_daifmt_parse_format(codec, NULL) |
		    snd_soc_daifmt_parse_clock_provider_as_flag(codec, NULL) |
	...
	struct snd_soc_dai_link *dai_link, struct sprd_card_data *priv)
{
	...
	/*
	 * dai_link->dpcm_playback/dpcm_capture are gone; direction validity
	 * now comes from the DAIs themselves, with playback_only /
	 * capture_only as the remaining overrides for one-directional links.
	 */
	{
		u32 has_play = 0, has_cap = 0;

		if (!of_property_read_u32(node, "dpcm-playback", val))
			has_play = !!val[0];
		if (!of_property_read_u32(node, "dpcm-capture", val))
			has_cap = !!val[0];
		dai_link->playback_only = has_play && !has_cap;
		dai_link->capture_only = has_cap && !has_play;
		pr_debug("%s dpcm-playback: %u dpcm-capture: %u\n", __func__,
			 has_play, has_cap);
	}

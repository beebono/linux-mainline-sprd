...
void *audio_mem_vmap(phys_addr_t start, size_t size, int writecombine)
{
	...
	/*
	 * vm_map_ram() lost its pgprot argument, and no-map reserved memory
	 * has no struct pages on arm64 anyway; memremap(MEMREMAP_WC) gives
	 * the uncached mapping the vendor kernel used for both the
	 * "noncached" and "writecombine" flavours.
	 */
	vaddr = memremap(page_start, (size_t)page_count * PAGE_SIZE,
			 MEMREMAP_WC);

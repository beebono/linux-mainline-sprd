...
static struct snd_soc_dai_ops sprd_fe_dai_compr_ops = {
	.probe = fe_dai_probe,
...
static int fe_dai_probe(struct snd_soc_dai *fe_dai)
{
	...
	dapm = snd_soc_component_to_dapm(fe_dai->component);
	...
	if (fe_dai->driver->playback.stream_name) {
		...
		intercon.sink = sprd_stream_aif_name(fe_dai->dev,
			fe_dai->driver->playback.stream_name);
	...
	if (fe_dai->driver->capture.stream_name) {
		...
		intercon.source = sprd_stream_aif_name(fe_dai->dev,
			fe_dai->driver->capture.stream_name);
		dev_dbg(fe_dai->dev, "%s src %s sink %s\n",
			__func__, intercon.source, intercon.sink);
		snd_soc_dapm_add_routes(dapm, &intercon, 1);
	}
...
static void sprd_fe_dai_dev_remove(struct platform_device *pdev)
{

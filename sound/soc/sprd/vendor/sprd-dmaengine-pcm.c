...
static int sprd_pcm_open(struct snd_pcm_substream *substream)
{
	...
	pr_info("%s %s Open %s\n", __func__, sprd_dai_pcm_name(snd_soc_rtd_to_cpu(srtd, 0)),
			PCM_DIR_NAME(substream->stream));

	if (sprd_is_i2s_or_tdm(snd_soc_rtd_to_cpu(srtd, 0))) {
		snd_soc_set_runtime_hwparams(substream, &sprd_i2s_pcm_hardware);
		if (snd_soc_rtd_to_cpu(srtd, 0)->driver->id == I2S_MAGIC_ID) {
			...
			tdmconf = sprd_tdm_dai_to_config(snd_soc_rtd_to_cpu(srtd, 0));
			if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK)
	...
	} else if (be_rtd && snd_soc_rtd_to_cpu(be_rtd, 0)->id == AGCP_IIS0_TX) {
		snd_soc_set_runtime_hwparams(substream, &sprd_i2s_pcm_hardware);
		burst_len = 0xc0 * 4;
		hw_chan = 1;
	} else {
	...
	if (snd_soc_rtd_to_cpu(srtd, 0)->id == FE_DAI_ID_NORMAL_AP01 &&
	...
	struct snd_pcm_substream *substream, int ch_cnt)
{
	...
	for (i = 0; i < ch_cnt; i++) {
		...
		dma_chn_request = dma_request_slave_channel(platform->dev,
			       ...
			       dma_addr_t *dma_buff_phys)
{
	...
	for (i = 0; i < ch_cnt; i++) {
		...
		/*
		 * .slave_id is gone; the sprd-dma channel already got its
		 * hardware slave/request id from the DT dmas cell when the
		 * channel was requested by name.
		 */
		if (is_playback) {
			...
			if (snd_soc_rtd_to_cpu(srtd, 0)->id ==
		...
		pr_info("src_dw=%d, dst_dw=%d frag=%lx, burst:%d, slave_id=%d, step=%d, ll_cfg_virt_addr=%p, ll_cfg_phy_addr=%ld ,dst_addr:%ld, src_addr:%ld,chan=%d\n",
			...
			cfg->dma_config_flag,
			(int)cfg->config.src_maxburst, dma_data->channels[i],
...
/* snd_soc_component_driver lost .ops; wrap the legacy callbacks. */
static int sprd_pcm_comp_open(struct snd_soc_component *component,
			      struct snd_pcm_substream *substream)
{
	return sprd_pcm_open(substream);
}

static int sprd_pcm_comp_close(struct snd_soc_component *component,
			       struct snd_pcm_substream *substream)
{
	return sprd_pcm_close(substream);
}

static int sprd_pcm_comp_hw_params(struct snd_soc_component *component,
				   struct snd_pcm_substream *substream,
				   struct snd_pcm_hw_params *params)
{
	return sprd_pcm_hw_params(substream, params);
}

static int sprd_pcm_comp_hw_free(struct snd_soc_component *component,
				 struct snd_pcm_substream *substream)
{
	return sprd_pcm_hw_free(substream);
}

static int sprd_pcm_comp_prepare(struct snd_soc_component *component,
				 struct snd_pcm_substream *substream)
{
	return sprd_pcm_prepare(substream);
}

static int sprd_pcm_comp_trigger(struct snd_soc_component *component,
				 struct snd_pcm_substream *substream, int cmd)
{
	return sprd_pcm_trigger(substream, cmd);
}

static snd_pcm_uframes_t
sprd_pcm_comp_pointer(struct snd_soc_component *component,
		      struct snd_pcm_substream *substream)
{
	return sprd_pcm_pointer(substream);
}

static int sprd_pcm_comp_mmap(struct snd_soc_component *component,
			      struct snd_pcm_substream *substream,
			      struct vm_area_struct *vma)
{
	return sprd_pcm_mmap(substream, vma);
}
						 ...
						 int stream)
{
	...
	if (sprd_is_i2s_or_tdm(snd_soc_rtd_to_cpu(rtd, 0)) ||
		(be_rtd && snd_soc_rtd_to_cpu(be_rtd, 0)->id == AGCP_IIS0_TX))
...
static int sprd_pcm_comp_new(struct snd_soc_component *component,
			     struct snd_soc_pcm_runtime *rtd)
{
	return sprd_pcm_new(rtd);
}

static void sprd_pcm_comp_free(struct snd_soc_component *component,
			       struct snd_pcm *pcm)
{
	sprd_pcm_free_dma_buffers(pcm);
}
...
static struct snd_soc_component_driver sprd_soc_platform = {
	...
	.open = sprd_pcm_comp_open,
	.close = sprd_pcm_comp_close,
	.hw_params = sprd_pcm_comp_hw_params,
	.hw_free = sprd_pcm_comp_hw_free,
	.prepare = sprd_pcm_comp_prepare,
	.trigger = sprd_pcm_comp_trigger,
	.pointer = sprd_pcm_comp_pointer,
	.mmap = sprd_pcm_comp_mmap,
	.pcm_new = sprd_pcm_comp_new,
	.pcm_free = sprd_pcm_comp_free,
};

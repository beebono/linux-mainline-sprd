			  ...
			  struct snd_ctl_elem_value *ucontrol)
{
	...
	board_ext_pin_control(snd_soc_card_to_dapm(card), id, id + 1);
				  ...
				  struct smartamp_boost_data *boost_data)
{
	...
	if (of_property_read_bool(np, smartamp_boost_name)) {
		...
		for (i = 0; i < ARRAY_SIZE(gpio_maps); i++) {
			...
			ret = sprd_of_get_named_gpio(np, smartamp_boost_gpios, index);
			if (ret < 0) {
...
int sprd_asoc_board_comm_late_probe(struct snd_soc_card *card)
{
	...
	board_ext_pin_control(snd_soc_card_to_dapm(card), 0, BOARD_FUNC_MAX);

	for (i = 0; i < BOARD_FUNC_MAX; i++)
		snd_soc_dapm_ignore_suspend(snd_soc_card_to_dapm(card), func_name[i]);

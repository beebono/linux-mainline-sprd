...
int wcn_thread_setattr(unsigned dir, struct sched_attr *attr)
{
	...
	/*
	 * sched_setattr() is not exported to modules; approximate the vendor
	 * intent (raise the tx/rx thread to real-time FIFO) with the exported
	 * sched_set_fifo() helper. The detailed attr is ignored for now.
	 */
	sched_set_fifo(target);
	ret = 0;

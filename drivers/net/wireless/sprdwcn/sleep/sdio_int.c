...
static int pub_int_handle_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		if (ret < 0)
			...
			if (reg_test == pub_int_sts0.reg) {
				reg_test_cnt++;
			} else {
				WCN_INFO("PUB_INT_STS0-0x%x\n", pub_int_sts0.reg);
				reg_test_cnt = 0;
			}
			reg_test = pub_int_sts0.reg;
...
int sdio_pub_int_init(int irq)
{
	...
	atomic_set(&flag_pub_int_done, 1);
	sdio_int.pub_int_wakelock = wakeup_source_register(NULL, "pub_int_wakelock");
...
int sdio_pub_int_deinit(void)
{
	...
	wakeup_source_unregister(sdio_int.pub_int_wakelock);

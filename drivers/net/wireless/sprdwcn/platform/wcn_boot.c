...
static int __maybe_unused gnss_download_firmware(void)
{
...
static int __maybe_unused gnss_start_run(void)
{
	int ret = 0;
	unsigned int temp = 0;
	struct wcn_match_data *g_match_config = get_wcn_match_config();
...
{
	/*
	 * GNSS is deferred in this bring-up build (gnssmodem not staged, GNSS
	 * subsystem stubbed). The stock flow downloads GNSS before btwifi and
	 * aborts the whole power-on if GNSS fails, so skip it and signal
	 * completion unconditionally to let Wi-Fi/BT (btwifi) bring-up proceed.
	 */
	pr_info("%s: GNSS download skipped (deferred); proceeding to btwifi\n",
		__func__);
	complete(&marlin_dev->gnss_download_done);
}

#if 0 /* original GNSS download path, disabled while GNSS is deferred */
static void pre_gnss_download_firmware_orig(struct work_struct *work)
{

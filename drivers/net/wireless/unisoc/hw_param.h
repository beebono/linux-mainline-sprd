 ...
 * (SC2355) WCN combo. Ported from the Unisoc 5.4 vendor sc2355 driver
 * (hw_param.{c,h}, cmdevt.c). The struct layouts below are ABI: the firmware
 * consumes them byte-for-byte, so field types/order/padding must not change.
 *
 * Copyright 2021-2022 Unisoc (Shanghai) Technologies Co., Ltd
 */

#ifndef __SC23XX_HW_PARAM_H__
#define __SC23XX_HW_PARAM_H__

#include <linux/types.h>

struct sc23xx_dev;

/* Parser working state (not part of the firmware ABI). */
struct nvm_cali_cmd {
	s8 itm[64];
	s32 par[512];
	s32 num;
...
/* full wifi config struct (parse target; sec1/sec2 alias its head) */
struct wifi_conf_t {
	struct version_t version;
	struct board_config_t board_config;
	struct board_config_tpc_t board_config_tpc;
...
/*
 * Parse the board-config INI named @fw_name and download it to the chip as
 * CRC'd INI sections. Caller (transport) picks @fw_name for the chip variant.
 */
int sc23xx_load_hw_param(struct sc23xx_dev *sdev, const char *fw_name);

#endif /* __SC23XX_HW_PARAM_H__ */

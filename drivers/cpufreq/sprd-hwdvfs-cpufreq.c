...
static int sprd_cpufreq_info_init(struct sprd_cpufreq_info *info, int cpu)
{
	...
	/*
	 * On 4.14/5.4 the cpu-map "cluster" nodes set physical_package_id,
	 * but since ~5.16 those set cluster_id (package_id stays 0 without a
	 * "socket" level), so use topology_cluster_id() for the cluster index.
	 */
	info->clu_id = topology_cluster_id(cpu);
	info->cpu_id = cpu;

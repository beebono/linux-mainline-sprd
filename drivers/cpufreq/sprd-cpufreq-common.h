...
/*
 * Hardware DVFS clusters map 1:1 onto the SoC's CPU clusters. On 4.14 the
 * cpu-map "cluster" nodes set physical_package_id, but since ~5.16 those nodes
 * set cluster_id (package_id stays 0 without a "socket" level), so use
 * topology_cluster_id() to recover the per-cluster index.
 */
#define sprd_cpufreq_data(cpu) \
	cpufreq_datas[topology_cluster_id(cpu)]
#define is_big_cluster(cpu) (topology_cluster_id(cpu) != 0)

 ...
 */
static int sprd_hardware_cpufreq_set_boost(struct cpufreq_policy *policy,
					   int state)
{
					   ...
					   unsigned int idx)
{
	...
	if ((boot_done_timestamp &&
		...
		sprd_hardware_cpufreq_set_boost(policy, 0);
	...
	cpu_cluster = topology_cluster_id(policy->cpu);
...
static int sprd_hardware_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	cpu = policy->cpu;
	curr_cluster = topology_cluster_id(cpu);
	...
	/* Hand the OPP-derived table to the cpufreq core */
...
static void sprd_hardware_cpufreq_exit(struct cpufreq_policy *policy)
{
	struct sprd_cpufreq_driver_data *data;
	struct sprd_cpudvfs_device *pdev;
	struct sprd_cpudvfs_ops *driver;
	...
	if (policy->freq_table)
		dev_pm_opp_free_cpufreq_table(data->cpu_dev,
					      &policy->freq_table);
	driver->enable(pdev->archdata,
		       topology_cluster_id(policy->cpu), false);
...
static void sprd_hardware_cpufreq_remove(struct platform_device *pdev)
{
	cpufreq_unregister_driver(&sprd_hardware_cpufreq_driver);
}

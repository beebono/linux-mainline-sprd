...
int sprd_dvfs_block_dcdc_shutdown_enable(struct cpudvfs_device *pdev, u32 num)
{
	...
	/*
	 * Deliberately optional on this board: asserting DVFS_BLOCK_SHUTDOWN
	 * while the DCDC_CPU1 external rail path is not working wedges the
	 * A75 cores (they stop answering IPIs and boot hangs). Omitting the
	 * property skips the write; see docs/A75-DVFS-REALME-PORT-PLAN.md.
	 */
	blk_sd_map = syscon_regmap_lookup_by_phandle_args(dcdc_node,
				"dvfs-blk-sd-syscon", 2, syscon_args);
	if (IS_ERR(blk_sd_map)) {
		dev_info(pdev->dev, "no dvfs block shutdown syscon, skipping\n");
		ret = 0;
		goto out;
	} else {
		reg = syscon_args[0];
...
static void sprd_cpudvfs_remove(struct platform_device *pdev)
{
	int ix;

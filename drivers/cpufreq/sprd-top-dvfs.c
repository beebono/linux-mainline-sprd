 ...
 *
 * Ported to mainline from vendor/linux-kernel-5-4-ums512 (same generation
 * stock T618 runs). It only brings the *device* DCDCs (mm, modem) and their
 * subsystems into hardware-DVFS mode; the cpu DCDCs are handled by
 * sprd-cpufreqhw. Kept a plain platform_driver (no devfreq framework use).
 */
...
static int topdvfs_device_dt_parse(struct topdvfs_dev *pdev)
{
	...
	if (!pdev->pwr) {
		pdev->pwr = kcalloc(nr, sizeof(struct dvfs_dcdc_pwr),
				    GFP_KERNEL);
		if (!pdev->pwr)
			return -ENOMEM;
	}
	...
	for (ix = 0; ix < nr; ix++) {
		...
		pdev->pwr[ix].subsys =
			kcalloc(subsys_num, sizeof(struct dcdc_subsys),
				GFP_KERNEL);
...
static void sprd_topdvfs_remove(struct platform_device *pdev)
{
}
